...
Deno.test({
  ...
  async fn(): Promise<void> {
    ...
    await Dami.listen();
    let cbResponse: DAMIData[] = [];
    const returnResponse: DAMIData[] = await Dami.to(
      "GetConfig",
      { Filename: "sip.conf", ActionID: 1234 },
    );
    await setTimeout(() => {
...
Deno.test({
  ...
  async fn(): Promise<void> {
    ...
    const res = await Dami.to(
      "GetConfig",
      { Filename: "rtp.conf", ActionID: 198 },
    );
    Rhum.asserts.assertEquals(
      res,
      [{
    ...
    const peerEntryResults: DAMIData[] = await Dami.to(
      "SIPPeers",
      { ActionID: 12354 },
    );
    await setTimeout(() => {
