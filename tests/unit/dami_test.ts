...
Rhum.testPlan("tests/unit/dami_test.ts", () => {
  ...
  Rhum.testSuite("connect()", () => {
    ...
    Rhum.testCase("Returns the auth/connect events when successfully connected", async () => {
      const Dami = new DAMI(ami);
    ...
    Rhum.testCase("Throws an error when auth creds are invalid", async () => {
      ...
      Rhum.asserts.assertEquals(errMsg, 'Authentication failed. Unable to login. Check your username and password are correct.')
    })
  });
  Rhum.testSuite("ping()", () => {
  ...
  Rhum.testSuite("to()", () => {
    Rhum.testCase("Throws an error when the action is  wrong because an event holds an error", async () => {
      const Dami = new DAMI(ami);
      await Dami.connect(auth);
      let err = false;
      let errMsg =  ""
